...
static void c63_encode_image(struct c63_common *cm, yuv_t *image)
{
    ...
    if (!cm->curframe->keyframe)
    {
        ...
        int end01 = clock();
        printf("[[[c63_motion_estimate: %d, c63_motion_compensate: %d]]], ", end00-start00, end01-start01);
    }
    ...
    int end2 = clock();
    printf("dct_quantize: %d, dequantize_idct: %d.\n", end1-start1, end2-start2);
...
int main(int argc, char **argv)
{
    ...
    while(!feof(infile))
    {
        ...
        fprintf(stderr, "Done!clock tick: %d\n", end-begin);
